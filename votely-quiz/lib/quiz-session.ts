...
export interface QuizSession {
  ...
  skippedQuestions?: number[]; // Track which questions were skipped
  phase1Scores?: {
    economic: number;
    governance: number;
    social: number;
...
export function saveQuizSession(session: QuizSession): void {
  ...
  try {
    ...
    debugError('Failed to save quiz session:', error);
  }
}
...
export function getCurrentSession(): QuizSession | null {
  ...
  try {
    ...
    debugError('Failed to load quiz session:', error);
  }
  ...
  try {
    ...
    if (currentSession) {
      ...
      if (parsed.sessionId === sessionId) {
        debugLog('Found session in sessionStorage');
        return parsed;
      }
    }
    ...
    if (session) {
      debugLog('Found session in localStorage');
      return session;
    }
    ...
    debugError('Failed to get session by ID:', error);
    return null;
  }
}
...
export function updateSessionAnswers(answers: Record<number, number | null>): void {
...
export function updateSessionPhase1Results(
  scores: { economic: number; governance: number; social: number },
  macroCellCode: string,
  tiebreakerBoundaries?: string[]
): void {
  const session = getCurrentSession();
...
export function generateShareableUrl(sessionId: string): string {
  ...
  const shareData = {
    ...
    a: Object.values(session.answers).filter(v => v !== null).map(v => Math.round(v! * 100)), // answers as integers 0-100
    t: session.type === 'long' ? 'l' : 's',
    m: session.macroCellCode || '',
    p: session.phase1Scores ? [
      Math.round(session.phase1Scores.economic),
      Math.round(session.phase1Scores.governance),
      Math.round(session.phase1Scores.social)
    ] : []
  };
...
export function parseShareableUrl(shareParam: string): {
  ...
  type: 'short' | 'long';
  macroCellCode?: string;
  phase1Scores?: { economic: number; governance: number; social: number };
} | null {
  try {
    ...
    return {
      ...
      phase1Scores: decoded.p?.length === 3 ? {
        ...
        governance: decoded.p[1],
        social: decoded.p[2]
      } : undefined
    };
  } catch (error) {
    debugError('Failed to parse shareable URL:', error);
    return null;
  }
}
